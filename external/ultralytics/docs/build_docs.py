...
def build_docs(clone_repos=True):
    ...
    if clone_repos:
        ...
        shutil.rmtree(
            DOCS / "en/hub/sdk", ignore_errors=True
        )  # delete if exists
        shutil.copytree(local_dir / "docs", DOCS / "en/hub/sdk")  # for docs
        shutil.rmtree(
            DOCS.parent / "hub_sdk", ignore_errors=True
        )  # delete if exists
        shutil.copytree(
    ...
    subprocess.run(
        f"mkdocs build -f {DOCS.parent}/mkdocs.yml", check=True, shell=True
    )
...
def main():
    ...
    # Update titles
    update_page_title(
        SITE / "404.html", new_title="Ultralytics Docs - Not Found"
    )
