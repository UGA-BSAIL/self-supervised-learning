...
class Exporter:
    ...
    def __call__(self, model=None):
        ...
        if any(f):
            ...
            predict_data = (
                f"data={data}"
                if model.task == "segment" and fmt == "pb"
                else ""
            )
            q = (
    ...
    def export_openvino(self, prefix=colorstr("OpenVINO:")):
        ...
        def serialize(ov_model, file):
            ...
            ov_model.set_rt_info(
                True, ["model_info", "reverse_input_channels"]
            )
            ov_model.set_rt_info(114, ["model_info", "pad_value"])
            ov_model.set_rt_info([255.0], ["model_info", "scale_values"])
            ov_model.set_rt_info(
                self.args.iou, ["model_info", "iou_threshold"]
            )
            ov_model.set_rt_info(
            ...
            ov.runtime.save_model(
                ov_model, file, compress_to_fp16=self.args.half
            )
            yaml_save(
    ...
    def export_ncnn(self, prefix=colorstr("NCNN:")):
        ...
        f = Path(
            str(self.file).replace(self.file.suffix, f"_ncnn_model{os.sep}")
        )
        ...
        if not pnnx.is_file():
            ...
            asset = attempt_download_asset(
                url, repo="pnnx/pnnx", release="latest"
            )
    ...
    def export_coreml(self, prefix=colorstr("CoreML:")):
        ...
        check_requirements(
            "coremltools>=6.0,<=6.2" if mlmodel else "coremltools>=7.0"
        )
        ...
        if self.args.nms and self.model.task == "detect":
            if mlmodel:
                ...
                check_version(
                    PYTHON_VERSION, "<3.11", name="Python ", hard=True
                )
                weights_dir = None
            else:
                ct_model.save(
    ...
    def export_saved_model(self, prefix=colorstr("TensorFlow SavedModel:")):
        ...
        if not onnx2tf_file.exists():
            attempt_download_asset(
                f"{onnx2tf_file}.zip", unzip=True, delete=True
            )
