...
class PoseValidator(DetectionValidator):
    ...
    def __init__(
        ...
        if (
            isinstance(self.args.device, str)
            and self.args.device.lower() == "mps"
        ):
            LOGGER.warning(
    ...
    def plot_predictions(self, batch, preds, ni):
        ...
        pred_kpts = torch.cat(
            [p[:, 6:].view(-1, *self.kpt_shape) for p in preds], 0
        )
        plot_images(
            batch["img"],
        ...
        if self.args.save_json and self.is_coco and len(self.jdict):
            ...
            try:  # https://github.com/cocodataset/cocoapi/blob/master/PythonAPI/pycocoEvalDemo.ipynb
                ...
                for i, eval in enumerate(
                    [
                        COCOeval(anno, pred, "bbox"),
                        COCOeval(anno, pred, "keypoints"),
                    ]
                ):
                    if self.is_coco:
                        eval.params.imgIds = [
