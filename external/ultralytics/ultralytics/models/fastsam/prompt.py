...
class FastSAMPrompt:
    ...
    def plot(
        ...
        for ann in pbar:
            ...
            plt.axis("off")
            plt.savefig(
                save_path, bbox_inches="tight", pad_inches=0, transparent=True
            )
            plt.close()
    ...
    def fast_show_mask(
        ...
        if random_color:
            ...
            color = np.ones((n, 1, 1, 3)) * np.array(
                [30 / 255, 144 / 255, 1.0]
            )
    ...
    def retrieve(
        ...
        preprocessed_images = [
            preprocess(image).to(device) for image in elements
        ]
    ...
    def _crop_image(self, format_results):
        ...
        for _, mask in enumerate(annotations):
            ...
            cropped_boxes.append(
                self._segment_image(image, bbox)
            )  # save cropped image
