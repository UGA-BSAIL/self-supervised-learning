...
class HungarianMatcher(nn.Module):
    ...
    def forward(
        ...
        pred_scores = (
            ...
            if self.use_fl
            else F.softmax(pred_scores, dim=-1)
        )
...
def get_cdn_group(
    ...
    neg_idx = (
        torch.arange(
            total_num * num_group, dtype=torch.long, device=gt_bbox.device
        )
        + num_group * total_num
    )
    ...
    padding_cls = torch.zeros(
        bs, num_dn, dn_cls_embed.shape[-1], device=gt_cls.device
    )
    ...
    pos_idx = torch.stack(
        [map_indices + max_nums * i for i in range(num_group)], dim=0
    )

    map_indices = torch.cat(
        [map_indices + max_nums * i for i in range(2 * num_group)]
    )
