...
class DETRLoss(nn.Module):
    ...
    def _get_loss_bbox(self, pred_bboxes, gt_bboxes, postfix=""):
        ...
        loss[name_giou] = 1.0 - bbox_iou(
            pred_bboxes, gt_bboxes, xywh=True, GIoU=True
        )
    ...
    def _get_loss_aux(
        ...
        loss = torch.zeros(
            5 if masks is not None else 3, device=pred_bboxes.device
        )
        ...
        for i, (aux_bboxes, aux_scores) in enumerate(
            zip(pred_bboxes, pred_scores)
        ):
...
class RTDETRDetectionLoss(DETRLoss):
    ...
    def forward(
        self, preds, batch, dn_bboxes=None, dn_scores=None, dn_meta=None
    ):
        """
    ...
    def get_dn_match_indices(dn_pos_idx, dn_num_group, gt_groups):
        ...
        for i, num_gt in enumerate(gt_groups):
            if num_gt > 0:
                gt_idx = (
                    torch.arange(end=num_gt, dtype=torch.long) + idx_groups[i]
                )
