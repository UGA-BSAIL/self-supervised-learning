...
class Predictor(BasePredictor):
    ...
    def pre_transform(self, im):
        ...
        assert (
            len(im) == 1
        ), "SAM model does not currently support batched inference"
    ...
    def generate(
        ...
        for crop_region, layer_idx in zip(crop_regions, layer_idxs):
            ...
            for (points,) in batch_iterator(
                points_batch_size, points_for_image
            ):
            ...
            keep = torchvision.ops.nms(
                crop_bboxes, crop_scores, self.args.iou
            )  # NMS
    ...
    def setup_model(self, model, verbose=True):
        ...
        self.std = (
            torch.tensor([58.395, 57.12, 57.375]).view(-1, 1, 1).to(device)
        )
    ...
    def set_image(self, image):
        ...
        assert (
            len(self.dataset) == 1
        ), "`set_image` only supports setting one image!"
    ...
    def remove_small_regions(masks, min_area=0, nms_thresh=0.7):
        ...
        keep = torchvision.ops.nms(
            boxes.float(), torch.as_tensor(scores), nms_thresh
        )
