...
def on_predict_start(predictor: object, persist: bool = False) -> None:
    ...
    for _ in range(predictor.dataset.bs):
        ...
        ):  # only need one tracker for other modes.
            break
...
def on_predict_postprocess_end(
    predictor: object, persist: bool = False
) -> None:
    """
    ...
    for i in range(len(im0s)):
        ...
        if (
            not persist
            and predictor.vid_path[i if is_stream else 0] != vid_path
        ):
            tracker.reset()
        ...
        det = (
            (
                predictor.results[i].obb
                if is_obb
                else predictor.results[i].boxes
            )
            .cpu()
            .numpy()
        )
...
def register_tracker(model: object, persist: bool) -> None:
    ...
    """
    model.add_callback(
        "on_predict_start", partial(on_predict_start, persist=persist)
    )
    model.add_callback(
