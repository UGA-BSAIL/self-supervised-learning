...
class BaseDataset(Dataset):
    ...
    def cache_images(self, cache):
        ...
        with ThreadPool(NUM_THREADS) as pool:
            ...
            pbar = TQDM(
                enumerate(results), total=self.ni, disable=LOCAL_RANK > 0
            )
            for i, x in pbar:
                if cache == "disk":
                    ...
                    b += self.ims[i].nbytes
                pbar.desc = (
                    f"{self.prefix}Caching images ({b / gb:.1f}GB {cache})"
                )
            pbar.close()
    ...
    def cache_images_to_disk(self, i):
        ...
        if not f.exists():
            np.save(
                f.as_posix(), cv2.imread(self.im_files[i]), allow_pickle=False
            )
    ...
    def set_rectangle(self):
        ...
        bi = np.floor(np.arange(self.ni) / self.batch_size).astype(
            int
        )  # batch index
