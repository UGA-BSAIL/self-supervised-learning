...
class YOLODataset(BaseDataset):
    ...
    def get_labels(self):
        ...
        lengths = (
            (len(lb["cls"]), len(lb["bboxes"]), len(lb["segments"]))
            for lb in labels
        )
    ...
    def close_mosaic(self, hyp):
        ...
        hyp.copy_paste = (
            0.0  # keep the same behavior as previous v8 close-mosaic
        )
...
class ClassificationDataset(torchvision.datasets.ImageFolder):
    ...
    def __init__(self, root, args, augment=False, prefix=""):
        ...
        if augment and args.fraction < 1.0:  # reduce training fraction
            self.samples = self.samples[
                : round(len(self.samples) * args.fraction)
            ]
    ...
    def verify_images(self):
        ...
        with contextlib.suppress(
            FileNotFoundError, AssertionError, AttributeError
        ):
            cache = load_dataset_cache_file(
                path
            )  # attempt to load a *.cache file
            assert (
                cache["version"] == DATASET_CACHE_VERSION
