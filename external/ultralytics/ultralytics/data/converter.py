...
def convert_dota_to_yolo_obb(dota_root_path: str):
    ...
    def convert_label(
        image_name, image_width, image_height, orig_label_dir, save_dir
    ):
        ...
        with orig_label_path.open("r") as f, save_path.open("w") as g:
            ...
            for line in lines:
                ...
                normalized_coords = [
                    coords[i] / image_width
                    if i % 2 == 0
                    else coords[i] / image_height
                    for i in range(8)
                ]
                formatted_coords = [
    ...
    for phase in ["train", "val"]:
        ...
        for image_path in TQDM(image_paths, desc=f"Processing {phase} images"):
            ...
            h, w = img.shape[:2]
            convert_label(
                image_name_without_ext, w, h, orig_label_dir, save_dir
            )
...
def yolo_bbox2segment(im_dir, save_dir=None, sam_model="sam_b.pt"):
    ...
    for l in tqdm(
        ...
        sam_results = sam_model(
            im, bboxes=xywh2xyxy(boxes), verbose=False, save=False
        )
        l["segments"] = sam_results[0].masks.xyn

    save_dir = (
        Path(save_dir) if save_dir else Path(im_dir).parent / "labels-segment"
    )
    save_dir.mkdir(parents=True, exist_ok=True)
