...
class Explorer:
    ...
    def create_embeddings_table(
        ...
        if self.data is None:
            raise ValueError(
                "Data must be provided to create embeddings table"
            )
        ...
        choice_set = (
            choice_set if isinstance(choice_set, list) else [choice_set]
        )
        ...
        batch = dataset[0]
        vector_size = self.model.embed(batch["im_file"], verbose=False)[
            0
        ].shape[0]
    ...
    def _yield_batches(
        ...
        for i in tqdm(range(len(dataset))):
            ...
            batch["vector"] = (
                model.embed(batch["im_file"], verbose=False)[0]
                .detach()
                .tolist()
            )
            yield [batch]
    ...
    def query(
        ...
        if self.table is None:
            raise ValueError(
                "Table is not created. Please create the table first."
            )
    ...
    def similarity_index(
        ...
        if max_dist < 0.0:
            raise ValueError(
                f"max_dist must be greater than 0. Got {max_dist}"
            )
        ...
        features = (
            self.table.to_lance()
            .to_table(columns=["vector", "im_file"])
            .to_pydict()
        )
    ...
    def plot_similarity_index(
        ...
        sim_idx = self.similarity_index(
            max_dist=max_dist, top_k=top_k, force=force
        )
