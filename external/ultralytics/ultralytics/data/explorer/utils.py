...
def sanitize_batch(batch, dataset_info):
    ...
    batch["keypoints"] = (
        batch["keypoints"].tolist() if "keypoints" in batch else [[[]]]
    )
    return batch
...
def plot_query_result(similar_set, plot_labels=True):
    ...
    masks = (
        similar_set.get("masks")
        if similar_set.get("masks")[0] != empty_masks
        else []
    )
    kpts = (
    ...
    for i, imf in enumerate(images):
        ...
        imgs.append(
            LetterBox(plot_size, center=False)(image=im).transpose(2, 0, 1)
        )
        if plot_labels:
            ...
            if len(masks) > i and len(masks[i]) > 0:
                ...
                plot_masks.append(
                    LetterBox(plot_size, center=False)(image=mask)
                )
        ...
        batch_idx.append(
            np.ones(len(np.array(bboxes[i], dtype=np.float32))) * i
        )
