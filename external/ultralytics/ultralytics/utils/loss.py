...
class KeypointLoss(nn.Module):
    ...
    def forward(self, pred_kpts, gt_kpts, kpt_mask, area):
        ...
        return (
            kpt_loss_factor.view(-1, 1) * ((1 - torch.exp(-e)) * kpt_mask)
        ).mean()
...
class v8ClassificationLoss:
    ...
    def __call__(self, preds, batch):
        ...
        loss = torch.nn.functional.cross_entropy(
            preds, batch["cls"], reduction="mean"
        )
...
class v8OBBLoss(v8DetectionLoss):
    def __init__(self, model):
        ...
        self.bbox_loss = RotatedBboxLoss(
            self.reg_max - 1, use_dfl=self.use_dfl
        ).to(self.device)
        ...
        try:
            ...
            targets = torch.cat(
                (
                    batch_idx,
                    batch["cls"].view(-1, 1),
                    batch["bboxes"].view(-1, 5),
                ),
                1,
            )
            rw, rh = (
                targets[:, 4] * imgsz[0].item(),
                targets[:, 5] * imgsz[1].item(),
            )
            targets = targets[
                (rw >= 2) & (rh >= 2)
    ...
    def bbox_decode(self, anchor_points, pred_dist, pred_angle):
        ...
        return torch.cat(
            (dist2rbox(pred_dist, pred_angle, anchor_points), pred_angle),
            dim=-1,
        )
