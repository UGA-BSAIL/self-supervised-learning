...
def benchmark(
    ...
    for i, (
        ...
        export_formats().iterrows()
    ):  # index, (name, format, suffix, CPU, GPU)
        ...
        try:
            ...
            if i == 9:  # Edge TPU
                ...
                assert (
                    MACOS or LINUX
                ), "export only supported on macOS and Linux"
            ...
            if verbose:
                ...
                ), f"Benchmark failure for {name}: {e}"
...
class ProfileModels:
    ...
    def profile(self):
        ...
        for file in files:
            ...
            table_rows.append(
                self.generate_table_row(
                    file.stem, t_onnx, t_engine, model_info
                )
            )
            output.append(
            ...
            if path.is_dir():
                ...
                files.extend(
                    [
                        ...
                        for file in glob.glob(str(path / ext))
                    ]
                )
