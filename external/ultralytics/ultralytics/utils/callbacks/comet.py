...
def _should_log_confusion_matrix():
    ...
    return (
        os.getenv("COMET_EVAL_LOG_CONFUSION_MATRIX", "false").lower() == "true"
    )
...
def _should_log_image_predictions():
    ...
    return (
        os.getenv("COMET_EVAL_LOG_IMAGE_PREDICTIONS", "true").lower() == "true"
    )
...
def _fetch_trainer_metadata(trainer):
    ...
    train_num_steps_per_epoch = (
        len(trainer.train_loader.dataset) // trainer.batch_size
    )
    ...
    save_assets = (
        save and save_period > 0 and save_interval and not final_epoch
    )
...
def _scale_bounding_box_to_original_image_shape(
    ...
    box = ops.scale_boxes(
        resized_image_shape, box, original_image_shape, ratio_pad
    )
...
def _log_images(experiment, image_paths, curr_step, annotations=None):
    ...
    if annotations:
        ...
        for image_path in image_paths:
            experiment.log_image(
                image_path, name=image_path.stem, step=curr_step
            )
...
def on_train_epoch_end(trainer):
    ...
    if curr_epoch == 1:
        _log_images(
            experiment, trainer.save_dir.glob("train_batch*.jpg"), curr_step
        )
