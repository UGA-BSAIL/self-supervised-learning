    ...
    if not current:  # if current is '' or None
        ...
        try:
            ...
            current = metadata.version(
                current
            )  # get version string from package name
...
def check_torchvision():
    ...
    v_torchvision = ".".join(
        torchvision.__version__.split("+")[0].split(".")[:2]
    )
...
def check_yolov5u_filename(file: str, verbose: bool = True):
    ...
    if "yolov3" in file or "yolov5" in file:
        if "u.yaml" in file:
            file = file.replace(
                "u.yaml", ".yaml"
            )  # i.e. yolov5nu.yaml -> yolov5n.yaml
...
def collect_system_info():
    ...
    for r in parse_requirements(package="ultralytics"):
        try:
            ...
            is_met = (
                "✅ "
                if check_version(current, str(r.specifier), hard=True)
                else "❌ "
            )
...
# Define constants
IS_PYTHON_3_12 = check_version(
    PYTHON_VERSION, "==3.12", name="Python ", hard=False
)
