...
AGENT_NAME = (
    f"python-{__version__}-colab"
    if is_colab()
    else f"python-{__version__}-local"
)
...
class HUBTrainingSession:
    ...
    def create_model(self, model_args):
        ...
        payload = {
            ...
            "lineage": {
                "architecture": {
                    "name": self.filename.replace(".pt", "").replace(
                        ".yaml", ""
                    ),
                },
                "parent": {},
        ...
        def retry_request():
            ...
            for i in range(retry + 1):
                if (time.time() - t0) > timeout:
                    LOGGER.warning(
                        f"{PREFIX}Timeout for request reached. {HELP_MSG}"
                    )
            ...
            if response is None and kwargs.get("metrics"):
                self.metrics_upload_failed_queue.update(
    ...
    def upload_metrics(self):
        ...
        return self.request_queue(
            self.model.upload_metrics,
            metrics=self.metrics_queue.copy(),
            thread=True,
        )
