...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, ch=(), bn_slices=4):
        ...
        self.cv2 = nn.ModuleList(
            nn.Sequential(
                Conv(x, c2, 3, bn_slices=bn_slices),
                Conv(c2, c2, 3, bn_slices=bn_slices),
        ...
        self.cv3 = nn.ModuleList(
            nn.Sequential(
                Conv(x, c3, 3, bn_slices=bn_slices),
                Conv(c3, c3, 3, bn_slices=bn_slices),
...
class Segment(Detect):
    ...
    def __init__(self, nc=80, nm=32, npr=256, ch=(), bn_slices=4):
        ...
        self.cv4 = nn.ModuleList(
            nn.Sequential(
                Conv(x, c4, 3, bn_slices=bn_slices),
                Conv(c4, c4, 3, bn_slices=bn_slices),
...
class OBB(Detect):
    ...
    def __init__(self, nc=80, ne=1, ch=(), bn_slices=4):
...
class Pose(Detect):
    ...
    def __init__(self, nc=80, kpt_shape=(17, 3), ch=(), bn_slices=4):
...
class Classify(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, bn_slices=4):
        """Initializes YOLOv8 classification head with specified input and output channels, kernel size, stride,
        padding, and groups.
        """
        super().__init__()
        c_ = 1280  # efficientnet_b0 size
        self.conv = Conv(c1, c_, k, s, p, g, bn_slices=bn_slices)
...
class WorldDetect(Detect):
    def __init__(self, nc=80, embed=512, with_bn=False, ch=(), bn_slices=4):
        ...
        self.cv4 = nn.ModuleList(
            BNContrastiveHead(embed, bn_slices=bn_slices)
            if with_bn
...
class RTDETRDecoder(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            bn_slices (int): Number of slices to use for shuffling BN.
        """
        super().__init__()
        self.hidden_dim = hd
        self.nhead = nh
        ...
        self.input_proj = nn.ModuleList(
            nn.Sequential(
                ...
                ShufflingBatchNorm2d(hd, num_slices=bn_slices),
            )
            for x in ch
        )
