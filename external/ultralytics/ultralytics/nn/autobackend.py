...
class AutoBackend(nn.Module):
    ...
    def __init__(
        ...
        fp16 &= (
            pt or jit or onnx or xml or engine or nn_module or triton
        )  # FP16
        nhwc = (
        ...
        # OpenVINO
            ...
            inference_mode = (
                "CUMULATIVE_THROUGHPUT" if batch > 1 else "LATENCY"
            )
            LOGGER.info(
                f"Using OpenVINO {inference_mode} mode for batch-size={batch_size} inference..."
            )
        ...
        # NCNN
            ...
            if not w.is_file():  # if not *.param
                w = next(
                    w.glob("*.param")
                )  # get *.param file from *_ncnn_model dir
    ...
    def forward(self, im, augment=False, visualize=False, embed=None):
        ...
        if self.pt or self.nn_module:
            y = self.model(
                im, augment=augment, visualize=visualize, embed=embed
            )
        ...
        # OpenVINO
            ...
            if self.inference_mode in {
                ...
                async_queue = self.ov.runtime.AsyncInferQueue(
        ...
        # NCNN
            ...
            input_names, output_names = (
                self.net.input_names(),
                self.net.output_names(),
            )
