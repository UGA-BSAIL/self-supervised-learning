...
class BodyQueryArtifactsQueryPost(BaseModel):
    """
    BodyQueryArtifactsQueryPost
    """  # noqa: E501

    queries: Optional[List[ImageQuery]] = None
    orderings: Optional[List[Ordering]] = None
    __properties: ClassVar[List[str]] = ["queries", "orderings"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in orderings (list)
        _items = []
        if self.orderings:
            for _item in self.orderings:
                if _item:
                    _items.append(_item.to_dict())
            _dict["orderings"] = _items
        return _dict
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate(
            {
                "queries": [
                    ImageQuery.from_dict(_item) for _item in obj.get("queries")
                ]
                if obj.get("queries") is not None
                else None,
                "orderings": [
                    Ordering.from_dict(_item) for _item in obj.get("orderings")
                ]
                if obj.get("orderings") is not None
                else None,
            }
        )
        return _obj
